...
def send_violation_report(channel_id, message, violation_type):
    """إرسال تقرير لمجموعة التقارير الخاصة بالقناة مع تنظيف الـ ID واستخدام HTML"""
    import re
    from ste import report_groups

    # تنظيف الـ ID
    clean_id = str(channel_id)
    clean_id = re.sub(r'[^0-9\-]', '', clean_id)
    if not clean_id.startswith('-100'):
        clean_id = f"-100{clean_id.lstrip('-')}"

    report_group_id = report_groups.get(clean_id)
    if not report_group_id:
        print(f"لم يتم العثور على مجموعة تقارير للقناة: {clean_id} (أصلي: {channel_id})")
        return

    # تنظيف النصوص لتجنب كسر HTML
    ...
    chat_title = escape_html(message.chat.title) if message.chat.title else "قناة مجهولة"
    chat_username = message.chat.username if message.chat.username else None
    chat_link = f"https://t.me/{chat_username}" if chat_username else "لا يوجد رابط"
    violation_type = escape_html(violation_type)

    # استخدام HTML بدلاً من Markdown
    report_text = (
        f"<b>تقرير مخالفة في القناة</b>\n"
        f"<b>القناة:</b> {chat_title}\n"
        f"<b>الرابط:</b> {chat_link}\n"
        f"<b>المخالفة:</b> {violation_type}\n"
    ...
    try:
        bot.send_message(
            report_group_id,
            report_text,
            parse_mode="HTML",  # ← غيّر إلى HTML
            disable_web_page_preview=True
        )
        print(f"تم إرسال التقرير بنجاح إلى المجموعة {report_group_id} للقناة {clean_id}")
    except Exception as e:
        print(f"خطأ في إرسال التقرير إلى {report_group_id}: {str(e)}")
