...
def send_violation_report(channel_id, message, violation_type):
    """إرسال تقرير لمجموعة التقارير الخاصة بالقناة مع تنظيف الـ ID وتجنب تداخل tqdm"""
    import re
    from ste import report_groups  # تأكد من الاستيراد إذا لزم

    # تنظيف الـ ID من أي بيانات إضافية (tqdm، أرقام، إلخ)
    clean_id = str(channel_id)
    clean_id = re.sub(r'[^0-9\-]', '', clean_id)  # اترك فقط الأرقام والـ -
    ...
    chat_title = message.chat.title if message.chat.title else "قناة مجهولة"
    ...
    try:
        ...
        print(f"تم إرسال التقرير إلى المجموعة {report_group_id} للقناة {clean_id}")
    except Exception as e:
