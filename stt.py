...
def load_games():
    ...
    for row in c.fetchall():
        ...
        active_games[game_id] = {
            ...
            'player_ids': [None, None] if mode == 'bot' else json.loads(players),  # تخزين معرفات اللاعبين
...
def update_leaderboard(winner=None, loser=None, is_draw=False, players=None, mode='pvp'):
    if mode != 'pvp':
        return  # لا تحديث للنقاط في وضع البوت
    conn = sqlite3.connect('chess_games.db')
    c = conn.cursor()
...
def check_sub_callback(call):
    ...
    if check_subscription(user_id):
        ...
        bot.send_message(chat_id, "🛡️ اختر وضع اللعب:", reply_markup=markup)
    else:
...
- /chess_challenge: تحدي لاعب في المجموعة
- /leaderboard: عرض أفضل 5 لاعبين بناءً على النقاط
- /help: عرض هذا الدليل
