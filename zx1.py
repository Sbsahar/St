...
import logging  # جديد: لتسجيل الأخطاء
...
# إعدادات البوت
TOKEN = '7937617884:AAGulic7N5PeqEYWdm3Trs_qbVn3gT-fM8k'  # استبدل بتوكن جديد إذا لزم
...
# جديد: تحميل نموذج كاشف العنف (مع تعليق مؤقت لتجنب تأخير، أزل التعليق إذا أردت)
...
# التحقق من الاشتراك في القناة (نسخة واحدة فقط، مع دمج [DEBUG])
...
def start(message):
    ...
    print(f"[DEBUG] المستخدم {user_id} مشترك، عرض رسالة البدء")
    markup = telebot.types.InlineKeyboardMarkup()
...
def activate_bot(message):
    ...
    activations[chat_id] = {
        ...
        'violence_enabled': False  # افتراضيًا معطل
    }
    save_activations()
