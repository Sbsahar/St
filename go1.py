...
# دالة إنشاء النص مع ضبط المسافات وحجم الخط والارتفاع (مع حواف سوداء وخلفية سوداء شفافة)
def create_text_clip_pil(text, fontsize, color, bg_color, max_width, start_time, end_time, position):
    ...
    # رسم النص مع خلفية سوداء شفافة وحواف سوداء
    img = Image.new("RGBA", (img_width, img_height), (0, 0, 0, 0))
    draw = ImageDraw.Draw(img)
    current_h = 0

    for line_idx, (line, h) in enumerate(zip(lines, line_heights)):
        # رسم الخلفية السوداء الشفافة للسطر
        left = 0
        right = img_width
        top = current_h - 5
        bottom = current_h + h + 15
        bg_fill = (0, 0, 0, 128)  # أسود شفاف (alpha=128 لشفافية جزئية)
        draw.rectangle([left, top, right, bottom], fill=bg_fill)
        
        # رسم النص مع حواف سوداء
        line_segments = re.split(r'([A-Za-z0-9\s]+)', line)
...
# التوكن هنا
TOKEN = "7942028086:AAE-I3_aoVqQ7gNE2CfuPb4JeZw77eewV9s"
...
def send_welcome(message):
    ...
    welcome_message = (
        ...
        "- نصوص بخط نسخ عربي أنيق مع حواف سوداء وخلفية شفافة.\n"
        "- إمكانية تصحيح الترجمات بسهولة!\n\n"
...
def handle_video(message):
    ...
    user_data = {'video_path': None, 'audio_path': None, 'segments': None, 'language': None, 'translated_segments': None}
    ...
    try:
        ...
        if len(segments) == 0:
            print("No segments detected! Trying without specified language...")
            result = model.transcribe(
                audio_path,
                language=None,
                fp16=False,
            ...
            segments = result["segments"]
            print(f"Retry transcription without language: {len(segments)} segments found")
        
        # تصحيح اللهجة للنصوص العربية دون تجاهل أي segments
        filtered_segments = []
        for segment in segments:
            # تصحيح اللهجة للنصوص العربية
...
def handle_callback(call):
    ...
    try:
        if data == "extract":
            ...
            user_data['translated_segments'] = translated_segments  # حفظ الترجمات للتصحيح
            user_data_storage[(user_id, chat_id)] = user_data
        ...
        # تنظيف البيانات (لا تنظف هنا إلا بعد التصحيح)
        pass
...
def handle_correction(message):
    ...
    if text == '/done':
        # إنهاء التصحيح وإعادة بناء الفيديو
        bot.send_message(chat_id, "✅ *تم الانتهاء من التصحيحات! جارٍ إعادة بناء الفيديو...* ⏳", parse_mode='Markdown')
        video_path = user_data['video_path']
        translated_segments = user_data['translated_segments']
        final_video_path = add_subtitles(video_path, translated_segments, is_translation=True)
        with open(final_video_path, "rb") as video_file:
            bot.send_video(chat_id, video_file, caption="🎥 *إليك الفيديو المُصحح والمُحدث!* 😊", parse_mode='Markdown')
    ...
    # البحث عن الـ segment الأقرب
    translated_segments = user_data['translated_segments']
    updated = False
    for i, (start, end, _) in enumerate(translated_segments):
        if abs(start - target_time) < 3:  # تحمل 3 ثوانٍ
            translated_segments[i] = (start, end, new_text)
            updated = True
            break
    
    if updated:
        user_data['translated_segments'] = translated_segments
        user_data_storage[(user_id, chat_id)] = user_data
        bot.reply_to(message, f"✅ *تم تحديث الترجمة في {time_str} إلى: {new_text}*", parse_mode='Markdown')
    else:
