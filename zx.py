...
BANNED_GROUPS_FILE = "banned_groups.json"  # ملف جديد لحظر المجموعات
user_violations = {}
daily_reports = {}
activations = {}
banned_words = {}
banned_groups = set()  # قائمة بالمجموعات المحظورة
...
def start(message):
    ...
    try:
        ...
        bot.send_message(
            message.chat.id,
            "اهلا وسهلا بك في بوت الحماية الذكي. جربني الآن!",
            reply_markup=markup
        )
    ...
    try:
        ...
        for member in message.new_chat_members:
            if member.id == bot.get_me().id:
                ...
                # تحقق مما إذا كانت المجموعة محظورة
                if str(chat_id) in banned_groups:
                    bot.send_message(
                        chat_id,
                ...
                # إرسال تنبيه للمطور
                adder_name = message.from_user.first_name if message.from_user else "غير معروف"
                group_title = bot.get_chat(chat_id).title
                group_link = bot.get_chat(chat_id).invite_link if bot.get_chat(chat_id).invite_link else "غير متوفر"
                group_id = chat_id
                ...
                print(f"[DEBUG] أرسلت تنبيه إضافة مجموعة {chat_id} إلى المطور")
...
def ban_group(message):
    ...
    try:
        ...
        print(f"[ERROR] خطأ في معالجة /ban: {e}")
...
def unblock_group(message):
    ...
    try:
        ...
        print(f"[ERROR] خطأ في معالجة /unblock: {e}")
...
load_activations()
load_banned_words()
load_banned_groups()  # تحميل المجموعات المحظورة
